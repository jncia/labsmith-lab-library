# Bring a Three-Router Core onto One Link-State Area, with a User at Each End

Three routers in a line, r1 - r2 - r3, with an end host behind r1 and another behind r3. The links and loopbacks are addressed and the host-facing ports are addressed, but there is no routing protocol, so neither user can reach the other. You bring all three routers into one OSPF area 0.0.0.0 and advertise the two user subnets with them, then bring the loopbacks in and prove the two ends reach each other.

## Scenario

You run a small core made of three routers in a line: r1, r2 and r3. r1 connects to r2, and r2 connects to r3. Behind r1 sits one user's machine and behind r3 sits another, each on its own subnet and each already pointing at the router in front of it. The links, the loopbacks and both user-facing ports are addressed and up. What is missing is a routing protocol. Today each router reaches only the neighbours it shares a link with, so r1 and r3 cannot see each other and neither user can reach the other. The team does not want static routes. They want the routers to learn each other's networks from a link-state protocol, they want the two users able to reach each other, and they want proof that the two ends of the line can reach each other's loopbacks through r2. You will do it in two stages: first get the routers talking to each other and the two user subnets carried between them, then bring the loopbacks in.
...
If your own image is tagged differently, set `VJUNOS_ROUTER_IMAGE` before you deploy; the topology takes the tag from the environment and falls back to the default it names. The two end hosts need nothing from you at all: their image is public, your container runtime fetches it at the exact digest the topology pins, and their addresses and their route are set by the lines the topology runs when they start. You never sign in to them and you never configure them.
...
The nodes of this lab are `r1`, `r2`, `r3`, `h1` and `h2`. Some of them may be fixtures rather than devices you work on; the drawing below says which.
...
| h1 | a user's machine behind r1 |
| h2 | a user's machine behind r3 |

| Node | Interface | Faces | Address |
| --- | --- | --- | --- |
| r1 | ge-0/0/0 | r2 | 10.0.12.1/30 |
| r1 | ge-0/0/1 | h1 | 10.10.1.1/24 |
...
| r3 | ge-0/0/1 | h2 | 10.10.3.1/24 |
| r3 | lo0.0 | no link | 10.255.0.3/32 |
| h1 | eth1 | r1 | 10.10.1.11/24 |
| h2 | eth1 | r3 | 10.10.3.11/24 |
...
| r1 | ge-0/0/1 (to h1) | 10.10.1.1/24 |
| r3 | ge-0/0/0 (to r2) | 10.0.23.2/30 |
| r3 | ge-0/0/1 (to h2) | 10.10.3.1/24 |
| r3 | lo0.0 | 10.255.0.3/32 |
| h1 | eth1 (to r1) | 10.10.1.11/24, default route through 10.10.1.1 |
| h2 | eth1 (to r3) | 10.10.3.11/24, default route through 10.10.3.1 |
...
| User subnets advertised into the area | 10.10.1.0/24 behind r1, 10.10.3.0/24 behind r3 |
| Loopbacks advertised into the area | 10.255.0.1/32, 10.255.0.2/32, 10.255.0.3/32 |

## What is already built

Every link interface, every loopback and both user-facing ports are already addressed and up, and you should leave that alone. The addressing and design values tables show what is where. No routing protocol is configured on any router, and there are no static routes to carry the traffic. r1 can reach r2 on the r1-r2 link, and r3 can reach r2 on the r2-r3 link. Each user can reach the router in front of it and nothing beyond. Nothing goes further than one hop.

## Stage 1 — Bring up the routing area

Make r1, r2 and r3 exchange routing information over their point-to-point links, all in one OSPF area, 0.0.0.0, and advertise the two user subnets in the same area so that each end learns the other user's subnet from the protocol rather than from a static entry. Do not disturb any interface or any address, and do not configure the end hosts: they are already addressed and already point at their own router.
...
- r1 has an OSPF route to 10.10.3.0/24 and r3 has an OSPF route to 10.10.1.0/24.
- The user behind r1 can reach the user behind r3, and the other way round.
- The link interfaces on all three routers are still up.

Hint: Ask what each end router knew about the far link before, and what would have to reach it to change that. A neighbour only tells you about networks it has been told to announce, and it only becomes a neighbour if both sides agree on the area. A subnet with no router on the far side of it still has to be announced, and the way to announce one is not the same as the way to bring up a neighbour on it.
...
Make every router's loopback part of the same routing area so that the loopbacks are learned from the protocol, and the two ends of the line can reach each other's loopback across r2. Keep the single area and the two user subnets from stage 1, and do not add static routes.
